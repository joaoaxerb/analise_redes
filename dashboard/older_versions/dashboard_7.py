...
# Initialize DataFrames for two files
df1 = pd.DataFrame()
df2 = pd.DataFrame()
...
app.layout = html.Div(style={'backgroundColor': 'lightblue', 'text-align': 'center'}, children=[
    ...
    # Dropdown menu for selecting which file to analyze
    dcc.Dropdown(
        id='file-selector',
        options=[
...
# Callback to handle file upload and update DataFrames
...
def update_tab(tab_name, contents1, contents2, filename1, filename2, selected_file):
    ...
    if tab_name == 'tab1':
        ...
        return [
            ...
            dcc.Graph(
                figure=px.histogram(protocol_counts, y="Protocol", x="Percentage", title="Porcentagem de pacotes por Protocolo", labels={
                              'Protocol': 'Communication Protocol', 'Percentage': 'Percentage of Total'}, category_orders={"Protocol": protocol_counts_sorted["Protocol"].tolist()}),
            ),
            # Bar chart - Packets per device (Horizontal Bar Chart)
            dcc.Graph(
                figure=px.histogram(selected_df, y="Source", title="Pacotes por Aparelho", labels={
        ...
        inter_arrival_time_distribution.update_layout(
            ...
            xaxis=dict(title="Inter-Arrival Time")
        )
